...
public class BuscadorService {
	...
	// metodo que a apartir de una temática devuelva un subconjunto
	// con las direcciones asociadas a dicha temática
	
	
	public HashSet<Direccion> buscarDireccion(String tematica) {
		HashSet<Direccion> nuevas_direcciones = new HashSet<Direccion>();
		for(Direccion d:direcciones) {
			if(d.getTematica().equals(tematica)){				
				nuevas_direcciones.add(d);
			}
		}
